...
```{r}
raw_eu <- read_csv("~/GITHUB/oceanoptics2021/data/lab13/es_Sensor0052-20210804093113.csv", skip = 2) %>% 
  rename(date = `date [EST]`) %>% 
  mutate(date = mdy_hm(date)) %>% 
  pivot_longer(cols = c(2:8), names_to = "wl", values_to = "values")
...
```{r}
raw_lu <- read_csv("~/GITHUB/oceanoptics2021/data/lab13/lu_Sensor0052-20210804093043.csv", skip = 2) %>% 
  rename(date = `date [EST]`) %>% 
  mutate(date = mdy_hm(date)) %>% 
  pivot_longer(cols = c(2:8), names_to = "wl", values_to = "values")
